...
section('الاتّجاه والرموز');
{
  ...
  /* ══════════════════════════════════════════════════════════
     تلميحٌ واحد لا اثنان، ونصٌّ لا يُعيد ما تحته

     قال صاحب المشروع: «عند التحويم على عناصر كالسهام، القارئ
     لا يفهم بشكل جيد. وهناك تكرار.» وكان محقًّا في اثنتين:

     ١. **فقاعتان تظهران معًا**: `<title>` من المتصفّح فوق
        بطاقتنا، تُعيد الاسم والدرجة اللذين في رأس البطاقة.
     ٢. **بطاقة السهم تُعيد تعريف المعجم** المكتوب تحتها بسطر:
        «السهم نقطة محسوبة لا جِرم لها» — مرّتين في نَفَسٍ واحد.
        وكذلك المنزلة.

     والأدهى أن أوّل إصلاحٍ لي زرع تكرارًا ثالثًا: ألحقتُ
     `lots_deep` بالسهام، فإذا هو **مطابقٌ مئةً بالمئة** لِما
     في `reading.lots`. فالقياس قبل الإضافة، لا بعدها.
     ══════════════════════════════════════════════════════════ */
  section('التحويم: واحدٌ لا اثنان');

  const wjs = read('assets/wheel.js');
  ok(/hintEatTitles/.test(read('assets/hint.js')),
     '`hint.js` يبتلع `<title>` حين يتولّى الشرح');
  ok(/aria-label/.test(read('assets/hint.js')),
     'وينقل مضمونها إلى `aria-label` فلا يخسر قارئ الشاشة');
  ok(/hintEatTitles\(document\.body\)/.test(read('assets/hint.js')),
     'ويُعيدها بعد كل رسمٍ جديد — فالعجلة تُرسَم بعد التحميل');

  /* ولا يُنزَع `<title>` من المصدر: من جاء بلا سكربت يحتاجها */
  ok((wjs.match(/<title>/g) || []).length >= 8,
     '`<title>` باقية في الرسم لمن جاء بلا جافاسكربت');

  /* ــ ولا سطرَ في بطاقةٍ يُعيد تعريف مصطلحها ــ */
  const GL = fixture.glossary || {};
  const sim = (a, b) => {          /* تشابهٌ خشن يكفي لالتقاط النسخ */
    const A = new Set(String(a).split(/\s+/).filter(w => w.length > 2));
    const B = new Set(String(b).split(/\s+/).filter(w => w.length > 2));
    if (!A.size || !B.size) return 0;
    let n = 0; A.forEach(w => { if (B.has(w)) n++; });
    return n / Math.min(A.size, B.size);
  };
  let echoes = [];
  (wjs.match(/hint\([\s\S]{0,900}?data-term="([^"]+)"/g) || []).forEach(blk => {
    const term = (blk.match(/data-term="([^"]+)"/) || [])[1];
    const g = GL[term];
    const gt = typeof g === 'string' ? g : (g && g.text) || '';
    if (!gt) return;
    (blk.match(/'([^']{25,240})'/g) || []).forEach(raw => {
      const lit = raw.slice(1, -1).replace(/\$\{[^}]*\}/g, '');
      if (lit.length < 25) return;
      if (sim(lit, gt) > 0.55) echoes.push(`${term}: «${lit.slice(0, 46)}…»`);
    });
  });
  ok(echoes.length === 0,
     echoes.length ? `بطاقةٌ تُعيد معجمها: ${echoes[0]}`
                   : 'ولا بطاقة تُعيد تعريف مصطلحها المكتوب تحتها');

  /* ــ وبطاقة السهم تقول **ما هو** لا أين هو وحده ــ */
  ok(/Lt\.note/.test(wjs), 'وبطاقة السهم تبدأ بما يدلّ عليه السهم');
  ok(!/Lt\.read/.test(wjs),
     'ولا تحمل النصّ الطويل — فهو في جدول السهام، والتحويم ليس مقالًا');
...
/* ══════════════════════════════════════════════════════════════
   النقر على جِرمٍ في العجلة يفتح قراءته الكاملة

   طُلِب هذا صراحةً: «أن تكون العجلة تفاعلية، تمّ في موقع
   Astrotheme». وكان التحويم يُظهر بطاقةً موجزة، **والقراءة
   الكاملة في جدولٍ أسفلَ لسانٍ آخر** — فمن رأى كوكبه في
   العجلة لم يجد إليه سبيلًا إلّا أن يبحث عنه بنفسه.

   **والفحص هنا يقرأ الشيفرة لا يُشغّلها.** وذلك اعترافٌ بحدٍّ:
   `chart.html` يبني نفسه من نداءات الشبكة، وتشغيله في jsdom
   يحتاج تزييف الطلبات كلّها. فالمفحوص هو **الرباط**: أن الطريق
   من العجلة إلى النصّ موصولٌ في مواضعه الأربعة.
   ══════════════════════════════════════════════════════════════ */
section('العجلة تُنقَر لا تُحوَّم عليها فقط');
{
  const page = read('chart.html');
  const body = page.split('<script>').pop();

  ok(/function jumpToBody/.test(body), 'للنقر على الجِرم دالّةٌ تُوصِله');
  ok(/g\.pl\[data-body\]/.test(body),
     'والمستمع على `g.pl[data-body]` — وهو ما يحمله الجِرم في العجلة');
  ok(/GO_PANE\('bodies'\)/.test(body),
     'ويُبدّل اللسان أوّلًا: القراءة في لسانٍ غير لسان العجلة');
  ok(/requestAnimationFrame/.test(body),
     'ثم يبحث عن الصفّ **بعد** إظهار اللسان — فالمخفيّ لا يُمرَّر إليه');
  ok(/toggleOpen\(row\)/.test(body), 'ويفتح الصفّ إن كان مطويًّا');
  ok(/scrollIntoView/.test(body), 'ويُمرِّر إليه');
  ok(/HINT\.pinned = false/.test(body),
     'ويُغلق البطاقة العائمة — فلا معنى لها فوق النصّ الذي قُفِز إليه');

  /* `GO_PANE` كانت حبيسة `initTabs`، فلا تُنادى من خارجها */
  ok(/let GO_PANE = null/.test(body) && /GO_PANE = go;/.test(body),
     'و`go` رُفعت من حبس `initTabs` لتُنادى من العجلة');

  /* الوميض: القفزة وحدها تترك القارئ يسأل «أين وقعتُ؟» */
  const css = read('assets/style.css');
  ok(/tr\.flash>td/.test(css) && /falakFlash/.test(css),
     'والصفّ المقصود يومض، فلا يسأل القارئ أين وقع');
  ok(/g\.pl\{cursor:pointer\}/.test(css),
     'والجِرم يُعلن بمؤشّره أنه يُنقَر');
  ok(/@media print\{tr\.flash>td\{animation:none\}\}/.test(css),
     'ولا وميض على الورق');

  /* ولا يُكسَر فتحُ الصفوف القائم: المستمع نفسه يخدمهما */
  ok(/const g = e\.target\.closest && e\.target\.closest\('g\.pl\[data-body\]'\);/
     .test(body), 'والمستمع واحدٌ مفوَّض، لا ثانٍ يتسابق مع الأوّل');
}

/* ــ الخلاصة تُطبع في آخر الملفّ، فيبقى هذا القسم قبلها ــ */
