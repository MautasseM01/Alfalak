...
def test_translation_layer_degrades_to_arabic_and_never_to_blank():
    ...
    js_raw = open(_os.path.join(_root, "assets", "i18n.js"),
                  encoding="utf-8").read()
    # **التعليقات تُجرَّد قبل الفحص.** وقعتُ في هذا مرّتين الآن:
    # الحارس يصطاد التعليق الذي يشرح إزالةَ ما يمنعه. فالتعليق
    # يصف الشيفرة ولا يكونها، ومن فحصهما معًا فحص كلامًا لا عملًا.
    js = _re.sub(r"/\*[\s\S]*?\*/|//[^\n]*", "", js_raw)
    # **`[data-body]` ليس منها** — انظر أسفله: رُفع من المستثنى
    # لأن صفوفَ جدول الأجرام تحمله، فكان يُستثنى الجدولُ نفسه.
    for guard in ("input", "textarea", ".wheel"):
        assert guard in js, f"المشي يدخل «{guard}» — وهو بيانات لا أثاث"
    # ══════════════════════════════════════════════════════════
    # **و`select` ليس منها — وكان، وهو خطأٌ في هذا الاختبار نفسه.**
    #
    # كتبتُ الشرط أوّلًا فجمعتُ `select` مع `input` بحجّة
    # «قيمتُها بيانات المستخدم». **وذلك خلطٌ بين الحقل وقائمته**:
    # قيمةُ `input` يكتبها الزائر، وأمّا نصُّ `<option>` فنحن
    # كتبناه. فبقيت كلُّ قائمةٍ منسدلة عربيّةً في الموقع كلِّه —
    # «— اختر —» و«شهر» و«— كل الساعات —» — مهما بلغ القاموس.
    #
    # ولم يصطده هذا الاختبار، **بل كان يحرسه**: اختبارٌ يثبّت
    # خطأً أضرُّ من غياب اختبار.
    #
    # فالمحروس الآن هو المعنى لا الاسم: القيمة (`value`) لا
    # تُمَسّ، إذ المشي على عُقَد النصّ لا على السمات.
    # ══════════════════════════════════════════════════════════
    assert "'select'" not in js and '"select"' not in js, \
        "`select` مستثنًى — فتبقى القوائم المنسدلة عربيّةً أبدًا"
    assert "SHOW_TEXT" in js, \
        "المشي على السمات لا على النصّ — فتُكسَر قيم `option`"
    # ══════════════════════════════════════════════════════════
    # **ثلاثةُ عيوبٍ ظهرت بالقياس من الصفحة، فتُحرَس هنا**
    #
    # ١. `raw.replace(key, hit)` — المفتاح **مسوّى** والخام كما
    #    كُتب، وأكثرُ الفقرات سطران. فالمفتاح ليس نصًّا فرعيًّا
    #    من الخام، **فيسقط `replace` بلا خطأ ولا أثر**. وهو
    #    أخبثُ ما مرّ: لا يرمي ولا يُسجّل، والصفحةُ عربيّة
    #    ونصُّها مترجَمٌ في القاموس.
    #
    # ٢. `'i18nA' + 'aria-label'` — و`dataset` لا تقبل الشرطة،
    #    فيرمي **داخل** المشي فلا يتمّ، ولا يُنفَّذ ما بعده.
    #
    # ٣. المعجم كان يُطلَب بلا لسان، فتظهر التلميحات عربيّةً
    #    فوق كلماتٍ إنجليزية.
    # ══════════════════════════════════════════════════════════
    assert "raw.replace(key" not in js, (
        "المفتاح مسوّى والخام ليس كذلك — فالاستبدال يسقط صامتًا")
    # **والشرطُ يُصاغ على ما لا يلتبس.** أوّل صياغةٍ لي كانت
    # `"'i18nA' + a" not in js` — وهي تُطابق `'i18nA' + attr`
    # أيضًا، أي **تصطاد الإصلاح نفسه**. فيُفحَص المستعمَل لا
    # المكتوب: أن يمرّ اسمُ السمة على دالّةٍ تُسوّي الشرطة.
    assert "i18nBak(" in js and "dataset[i18nBak" in js, (
        "`aria-label` فيها شرطة و`dataset` لا تقبلها — فيرمي المشي")
    assert _re.search(r"function i18nBak[\s\S]{0,200}?replace\(/-", js), (
        "`i18nBak` لا تُسوّي الشرطة، فالاسم باطلٌ في `dataset`")
    hint_js = open(_os.path.join(_root, "assets", "hint.js"),
                   encoding="utf-8").read()
    assert "/api/glossary?lang=" in hint_js, (
        "المعجم يُطلَب بلا لسان — فتلميحٌ عربيّ فوق نصٍّ إنجليزيّ")

    # ــ **وما لم يُترجَم من المعجم يُسقَط ولا يُردّ عربيًّا** ــ
    from falak import glossary_i18n as _g
    from falak import interpret as _in
    for _lang in ("en", "fr"):
        for _name, _text in _g.terms_for(_lang, _in.GLOSSARY).items():
            assert not _re.search(r"[ء-ي]", _name + _text), (
                f"المعجم يردّ «{_name}» عربيًّا في صفحة «{_lang}» — "
    ...
    #
    # رُئي في المتصفّح: القائمةُ عربيّة، وفوقها صندوق «هذه
    # الصفحة ليست مترجمة» **بالإنجليزية**. وهو أسوأ ما يمكن:
    # القارئ اختار العربية، فيُقال له بلسانٍ آخر إنها غير مترجمة.
    #
    # والسبب سطرٌ واحد: خروجٌ مبكّر يُنادي `i18nBar()` وحدها.
    # **والخروجُ المبكّر بابُ نسيان** — كل ما يُضاف بعده لا
    # يُنفَّذ في ذلك الطريق. فيُنظَّف كلُّ ما وُضع، لا بعضُه.
    # ══════════════════════════════════════════════════════════
    ar_branch = js[js.index("I18N_LANG === 'ar'"):]
    ar_branch = ar_branch[:ar_branch.index("return;")]
    for must in ("i18nNotice()", "I18N_VOCAB = null", "I18N_NOTE = ''"):
        assert must in ar_branch, \
            f"العودة إلى العربية لا تُنظّف «{must}» — فتبقى آثارُ الأخرى"
    ...
    # «3° 28′ العذراء»، وهي ليست مفتاحًا ولن تكون — فالدرجة
    # تتبدّل في كل خريطة. فلو اكتُفي بالعبارات لبقيت الجداول
    # عربيّةً **وإن بلغ القاموس مئةً بالمئة**.
    # ══════════════════════════════════════════════════════════
    v = i18n.vocab_for("en")
    assert len(v) >= 35, "المفردات ناقصة، والجداول تُبنى عليها"
    for k in ("الشمس", "العذراء", "تثليث", "الطالع"):
        assert k in v and v[k] != k
    # ولا نصَّ يُكرَّر: المفردات تُؤخَذ من `UI` لا تُكتَب ثانيةً
    for k in i18n.VOCAB_KEYS:
        assert k in i18n.UI, f"«{k}» في المفردات ولا أصل له في القاموس"

    # ــ والحدُّ يمنع الخلط ــ
    # لو بُدِّلت المفردات داخل الفقرات لخرج «تُحبّ Venus صورةً
    # لا شخصًا». **والخلطُ أسوأ من العربيّة الصافية.**
    assert 30 <= i18n.VOCAB_MAX <= 60, "حدٌّ واسع يُفسد الفقرات"

    assert "vocab" in dispatch("/api/i18n", {"lang": ["en"]})
    assert dispatch("/api/i18n", {"lang": ["ar"]})["vocab"] == {}

    # ــ ولا يُبتلَع حرفُ عطف ــ
    # كتبتُ النمط أوّلًا يلتقط `([وفبكل]?)` ثم أُسقطه في التبديل،
    # فكانت «والشمس» تصير «Sun» بلا واو. **وابتلاعُ حرفٍ أسوأ
    # من تركِ كلمةٍ عربية.**
    assert "[وفبكل]" not in js, "النمط يلتقط حرف العطف ثم يُسقطه"
    assert "VOCAB" in js.upper(), "لا مرحلةَ مفرداتٍ في الواجهة"

    # ══════════════════════════════════════════════════════════
    # **الوسمُ قبل الترجمة — وإلّا ضاعت التلميحات كلُّها**
    #
    # قال صاحب المشروع: «اختفت التلميحات عند التحويم». وكان
    # محقًّا: `hint.js` يَسِم المصطلحات بالبحث عن **نصّها
    # العربي**، وكنّا نترجم أوّلًا ثم يَسِم هو — فيجد إنجليزيّةً
    # لا يعرفها. فقِيس ذلك: **صفرُ تلميحاتٍ في نتيجةٍ رُسمت
    # بالإنجليزية**، وهي أكثر الصفحة.
    #
    # والحلّ لا يكون بتأخير رقمٍ عن رقم — فمن بنى ترتيبًا على
    # مللي ثانية بنى على رمل. بل يُنادى الوسمُ صراحةً قبلها.
    # ══════════════════════════════════════════════════════════
    assert "i18nMarkFirst" in js, "لا وسمَ قبل الترجمة"
    assert js.count("i18nMarkFirst()") >= 2, \
        "الوسم يسبق الترجمة في موضعٍ واحد — والنتيجة تُرسَم في الآخر"
    body = js[js.index("MutationObserver"):]
    assert "i18nMarkFirst" in body, \
        "المراقب يترجم بلا وسم — ونتيجةُ الحساب تُرسَم من خلاله"

    # ــ وألفاظ الحال تُبدَّل داخل النصّ لا بمطابقته ــ
    # «تُرجم المُجمَل وبقيت كلمات، مثل الكوكب الراجع» — وهي
    # لا تقع وحدها في خليّة بل داخل: «14° 36′ الحمل (راجع)».
    for w in ("راجع", "الغريب", "نهارية"):
        assert w in i18n.VOCAB_KEYS, f"«{w}» تبقى عربيّةً وسط الإنجليزية"
    # و`[data-body]` رُفع من المستثنى: صفوفُ جدول الأجرام تحمله
    assert "'[data-body]'" not in js, \
        "الجدول مستثنًى من الترجمة التي بُنيت له"
    ...
    _sys.path.insert(0, _os.path.join(_root, "tools"))
    import i18n_todo
    # **والحساب من موضعٍ واحد.** كان هنا حسابٌ وفي الأداة حسابٌ
    # آخر، فقالت الأداة ١٠٠٪ وقال الاختبار ٩٩٫٧ — **وحاسبان
    # لشيءٍ واحد أحدُهما يكذب حتمًا**، ولا يُعرَف أيُّهما.
    done, total, todo = i18n_todo.coverage()
...
def test_daily_bulletin_shows_the_whole_sky():
    ...
    ٢٠٨٧ حرفًا في ستّة أقسام، **تدور على القمر وحده تقريبًا**.

    والناقص لم يكن محتاجًا إلى حسابٍ جديد — بل كان **محسوبًا عندنا
    في مواضع أخرى**: مواضع الأجرام في `chart`، وزوايا الكواكب في
    `find_aspects`، ودخول البروج في `mundane`. فما كان يرى الزائرُ
    إلّا القمر.

    > **وطبقة التبسيط كسرت سطرين من الجديد**، فأُصلحا في النصّ لا
    > في الطبقة: كتبتُ «مُقبِلة تشتدّ» فصارت «تشتدّ ولم تتمّ بعد
    > (مُقبِلة) تشتدّ»؛ وكتبتُ «الطالع ظهرًا» فصارت «البرج الصاعد
    > **لحظة الميلاد**» — وهو صحيحٌ في المولد **خطأٌ في نشرة اليوم**.
    """
    from api.index import dispatch
    from falak import bulletin_more

    r = dispatch("/api/bulletin", {"date": ["2026-08-09"], "city": ["دمشق"]})
    t = r["text"]

    assert len(t) >= 3200, f"النشرة ما زالت قصيرة ({len(t)} حرفًا)"
    for head in ("#سماء_اليوم", "#زوايا_الكواكب", "#ما_يقترب"):
        assert head in t, f"ينقص القسم {head}"

    sky = r.get("sky")
    assert sky and len(sky["bodies"]) >= 12, "لا تُعرَض الأجرام كلّها"
    assert sky.get("asc_noon")

    # زوايا الكواكب — لا زوايا القمر
    for p in sky["pairs"]:
        assert "القمر" not in (p["a"], p["b"]), "زوايا القمر لها قسمها"
        assert p.get("meaning"), f"{p['a']}–{p['b']}: زاوية بلا معنى"
    # والأقرب إلى الإنسان أوّلًا: لا تتصدّر زاويةُ خارجيَّين
    if len(sky["pairs"]) >= 2:
        FAST = {"الشمس", "عطارد", "الزهرة", "المريخ", "المشتري", "زحل"}
        first = sky["pairs"][0]
        assert first["a"] in FAST or first["b"] in FAST, \
            "تصدّرت زاويةٌ بين جِرمين خارجيَّين — وهي لا تتبدّل فتكون خبرًا"

    assert len(r.get("coming") or []) >= 3, "لا يُذكَر ما يقترب"

    # ــ وطبقة التبسيط لا تُنتج ركاكة ــ
    assert "(مُقبِلة) تشتدّ" not in t, "تكرار من طبقة التبسيط"
    assert "تشتدّ ولم تتمّ بعد (مُقبِلة) تشتدّ" not in t
    assert "الصاعد لحظة الميلاد" not in t, \
        "«الطالع» في نشرة اليوم فُسِّر بلحظة الميلاد — وهذا خطأ هنا"

    assert bulletin_more.coverage()["أقسام مُضافة"] == 3
